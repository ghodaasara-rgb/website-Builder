...
import _tmpl from "./sidePanel.html";
...
class SidePanel extends LightningElement {
  constructor(...args) {
    super(...args);
    this.DEFAULT_THEMES = [{
      value: 'corporate',
    ...
    this.components = [];
    // Receive components from parent builder
    this.pages = [];
    // Receive pages from parent builder
    ...
    // Branding Options
    this.themeLayoutOptions = [{
      label: 'Sidebar (Left Nav)',
      value: 'sidebar'
    }, {
      label: 'Top Nav',
      value: 'topnav'
    ...
    // Selected Values
    this.selectedThemeLayout = 'sidebar';
    this.selectedBrandSet = 'new_set';
    this.colors = {
      primary: '#5b5fc7',
      background: '#e5e5e5',
      text: '#444444'
  ...
  initializeBrandSets() {
    ...
    this.brandSetOptions = [{
      label: 'Select a Brand Set',
      value: ''
    }, ...this.DEFAULT_THEMES, ...customSets];
  }
  ...
  get isBuildTab() {
    return this.activeTab === 'panel-build';
  }
  get isThemeTab() {
    return this.activeTab === 'panel-theme';
  }
  get isStructureTab() {
    return this.activeTab === 'panel-structure';
  }
  get isPagesTab() {
    return this.activeTab === 'panel-pages';
  }
  get isSettingsTab() {
    return this.activeTab === 'panel-settings';
  }
  get isHeaderTab() {
    return this.activeTab === 'panel-header';
  }
  get isLogoTextOnly() {
  ...
  get showLogoText() {
    return this.headerConfig.logoType === 'text' || this.headerConfig.logoType === 'both';
  }
  get showLogoImage() {
    return this.headerConfig.logoType === 'image' || this.headerConfig.logoType === 'both';
  }
  ...
  handlePropertyChange(event) {
    ...
    this.dispatchEvent(new CustomEvent('propertychange', {
      detail: {
        ...
        // Added triggering field for debugging
        props: {
...
SidePanel.renderMode = 'light';
_registerDecorators(SidePanel, {
  publicProps: {
    activeTab: {
      config: 0
    },
  ...
  fields: ["DEFAULT_THEMES", "themeLayoutOptions", "fontOptions", "selectedThemeLayout", "selectedBrandSet", "colors", "typography"]
});
const __lwc_component_class_internal = _registerComponent(SidePanel, {
  tmpl: _tmpl,
  sel: "custom-side-panel",
  apiVersion: 66
});
