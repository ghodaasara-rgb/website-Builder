...
const BACKEND_BASE = import.meta.env && import.meta.env.API_URL || process.env.API_URL || 'https://backend-six-xi-72.vercel.app'; // Fallback to deployed backend
...
export async function loadPage(pageId) {
  try {
    ...
    const [pageRes, componentsRes] = await Promise.all([fetch(`${API_BASE_URL}/pages/${pageId}`), fetch(`${API_BASE_URL}/pages/${pageId}/components`)]);
