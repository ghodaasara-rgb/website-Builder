...
export class HistoryRouter extends DomRouterImpl {
  ...
  onpopstate() {
    ...
    if (!this.parent && hasDocument) {
      // Remove fragment identifier from URL to prevent navigation for fragment changes
      // Exception: hashes in a basePath allow for deep linking of URLs not supported on the server
      const relativeUrl = getRelativeUrl(window.location.href);
      const hashlessUrl = this.router.basePath.includes('#') ? relativeUrl : removeFragment(relativeUrl);
  ...
  async process(url, shouldReplace, options, updateHistory = true) {
    ...
    // Only the root router handles history and currentUrl tracking
    if (canContinue && this.connected && !this.parent) {
      // Always track the current URL for popstate comparison.
      // This must be updated for both programmatic navigation (updateHistory=true)
      // and browser-initiated navigation (updateHistory=false) to ensure
      // subsequent popstate events are correctly detected.
      this.currentUrl = url;

      // Only modify browser history for programmatic navigation
      if (!this.historyDisabled && updateHistory) {
        if (shouldReplace) {
          replace(url);
        } else {
          set(url);
