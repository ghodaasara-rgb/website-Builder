...
function createViewMiddleware(route, errorRoutes, context, viewHandler) {
    ...
    return async (req, res) => {
        ...
        try {
            ...
            // Set B3 tracing headers and other custom headers (correlation ID, true client IP)
            // B3 headers are now generated even when OpenTelemetry tracing is disabled, enabling
            // request correlation across microservices without full tracing overhead
            if (Object.keys(headers).length)
                res.set(headers);
            // Set Server-Timing header only when there are actual spans to report
            // This prevents empty Server-Timing headers when tracing is disabled but trace IDs are generated
            if (traceId?.length) {
                const serverTiming = getTraceCollector().getSpansInTrace(traceId);
                if (serverTiming) {
                    res.set({
