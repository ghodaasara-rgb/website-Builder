...
function secureExporter(
    ...
    // Allow for standard lwr specifiers (space/name) as well as aura style specifiers (space:name)
    const [namespace, name] = specifier.split(/[/:]/);

    // return the exporter as is if it's from a trusted component or namespace
    if (
        trustedNamespaces.has(namespace) ||
        trustedNamespaces.has(`${namespace}/*`) ||
        trustedNamespaces.has(`${namespace}/${name}`) ||
        trustedNamespaces.has(specifier)
    ) {
        return exporter;
    }
    ...
    if (
        exportsIndex !== -1 ||
        lwcIndex !== -1 ||
        platformResourceLoaderIndex !== -1 ||
        lmrPlatformResourceLoaderIndex !== -1
    ) {
        return (...args) => {
            if (exportsIndex !== -1) {
...
function registerLockerDefine(trustedNamespaces) {
    ...
    const trustedSet = new Set(trustedNamespaces);
        ...
        Object.assign(Object.assign({}, (globalThis ).LWR), {
            define: function (specifier, dependencies, exporter, signature) {
                ...
                loaderDefine(
                    specifier,
                    dependencies,
                    secureExporter(specifier, dependencies, exporter, trustedSet),
                    signature,
                );
            },
        }),
