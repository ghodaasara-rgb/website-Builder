...
async function expPreview(options) {
  ...
  try {
    ...
    if (proxyConfig) {
      ...
      await (0, import_utils.setupProxy)(runtimePort, proxyPort, appBasePath || "", {
        ...proxyConfig,
        remoteOrigin
      });
    }
      ...
      if (import_fs_extra.default.existsSync(dir)) {
        ...
        if (appBasePath) {
          try {
            ...
            const basePathDir = import_path.default.join(dir, appBasePath.replace(/^\//, ""));
            import_diagnostics.logger.info({
              label: `cli`,
            ...
            internalServer.use((req, res, next) => {
              try {
                if (req.originalUrl.startsWith(normalizedBasePath)) {
                  ...
                  const filePath = import_path.default.join(basePathDir, decodedPath);
