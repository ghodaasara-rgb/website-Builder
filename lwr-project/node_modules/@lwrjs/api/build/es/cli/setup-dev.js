...
export function injectLocalDevHacks(filePath) {
    try {
        const fileContent = fs.readFileSync(filePath, 'utf8');
        // Overrides the __Secure_has_id cookie when running a site locally
        const updatedContent = fileContent.replace(/(__Secure-has-sid|isLocalDev)/g, 'LocalAuthenticationEnabled');
        // Turn off image optimizaion when running a site locally
        const newestContent = updatedContent.replace(/return\s"Cloudflare";/, 'return "none";');
        fs.writeFileSync(filePath, newestContent, 'utf8');
    }
    catch (error) {
...
export async function setupDev(options) {
    ...
    injectLocalDevHacks(path.join(pathToAppFolder, 'experience', 'index_csr.html'));
    injectLocalDevHacks(path.join(pathToAppFolder, 'experience', 'index.html'));
    // Also need to update the individual authenticationCookieName.js files
    injectLocalDevHacks(path.join(pathToAppFolder, 'site', 'dev', 'module', '@app', 'authenticationCookieName.js'));
    injectLocalDevHacks(path.join(pathToAppFolder, 'site', 'prod', 'module', '@app', 'authenticationCookieName.js'));
    // 10. Turn off image optimization when running locally (since we don't want to try and replicate CDN behavior)
    // Other option would be to try and proxy all those cdn-cgi urls correctly? Not super keen on keeping up with that
    injectLocalDevHacks(path.join(pathToAppFolder, 'site', 'dev', 'module', '@app', 'imageOpt.js'));
    injectLocalDevHacks(path.join(pathToAppFolder, 'site', 'prod', 'module', '@app', 'imageOpt.js'));
}
