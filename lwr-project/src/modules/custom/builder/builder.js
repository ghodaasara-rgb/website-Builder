...
export default class Builder extends LightningElement {
    ...
    @track activeTab = 'panel-build';
    @track componentList = [];
    @track pagesList = []; // Pages loaded from database
    @track activePageId = null; // Currently active page ID
    @track isSaving = false;
    @track isLoading = false;
    @track notification = { show: false, message: '', type: '' };
    @track selectedComponentId = null;
    ...
    get isLogoText() { return this.headerConfig.logoType === 'text'; }
    get isLogoImage() { return this.headerConfig.logoType === 'image'; }
    ...
    handleHeaderChange(event) {
        this.headerConfig = { ...this.headerConfig, ...event.detail.headerConfig };
        ...
        try {
            ...
            if (pageData.layout && pageData.layout.sections) {
                this.componentList = pageData.layout.sections.map(section => ({
                    ...
                    isCard: section.type === 'card',
                    props: { ...this.getDefaultProps(section.type), ...(section.props || section.properties || {}) },
    ...
    buildPageState() {
        ...
        return {
            ...
            metadata: {
                ...
                createdAt: new Date().toISOString(), // This should ideally come from loaded data to preserve creation time
    ...
    handleThemeChange(event) {
        if (event.detail.theme) {
            this.theme = { ...this.theme, ...event.detail.theme };
        }
    }

    async handlePropertyChange(event) {
        const { componentId, props } = event.detail;
        ...
        this.componentList = this.componentList.map(comp => {
            if (comp.id.toString() === componentId.toString()) {
                const newProps = { ...comp.props, ...props };
                return {
                    ...comp,
                    props: newProps,
    ...
    async handlePageAdd(event) {
        ...
        try {
            ...
            await this.handlePageSelect({ detail: { pageId: newPage.id } });
    ...
    async handlePageRename(event) {
        const { pageId, name } = event.detail;
        ...
        try {
            ...
            await savePage(pageId, { name, slug });
